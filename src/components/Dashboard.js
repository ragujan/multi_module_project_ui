...
function Dashboard() {
  ...
  let sideBarClassList =
    "flex-col py-5 md:w-[250px] hidden md:flex gap-y-3 transistion duration-300";
  ...
  useEffect(() => {
    ...
    if (sideBarWidth === 0) {
      let value = bodySectionRef.current.querySelector('div:first-child').getBoundingClientRect().width;

      let paddingLeft = parseInt(window.getComputedStyle(bodySectionRef.current).paddingLeft);
      let paddingRight = parseInt(window.getComputedStyle(bodySectionRef.current).paddingRight);

      setSideBarWidth(sideBarRef.current.offsetWidth+2*(paddingLeft+paddingRight))

    }
    console.log("setting option height is " + settingOptionHeight);
    console.log("side bar with is ", sideBarWidth);
  }, [settingOptionHeight, sideBarState]);
  useEffect(() => {
    if (sideBarState) {
      sideBarRef.current.style.marginLeft = `-${sideBarWidth}px`;
    } else {
  ...
  return (
    <div className="flex flex-col bg-main-blue h-[100vh] ">
      ...
      {/* body section */}
      <div ref={bodySectionRef} className="flex flex-row px-3 bg-main-blue">
        {/* sidebar */}
        <div ref={sideBarRef} className={`${sideBarClassList}`}>
